...
export const Login = () => {
  ...
  const handleSubmit = (e) => {
    ...
    fetch(import.meta.env.VITE_REACT_SERVER_URI + "/login", requestOptions)
    .then(res => res.json())
    .then(res => {
      console.log(res)
      setToken(res.token)
    ...
    <Container>
      <div className='form'>
        <form onSubmit={handleSubmit}>
          <h1>Log In</h1>
          {loginError && <p className='login-error'><WarningIcon sx={{ width: "0.75em", height: "0.75em", color: "#de5757" }}/> {loginError}</p>}
          <FormInput
            name="email"
            type="text"
            autoComplete="true"
